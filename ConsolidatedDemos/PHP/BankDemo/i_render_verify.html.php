...
if(!isset($_SESSION['mac-bank']['loggedIn']) || (isset($_SESSION['mac-bank']['loggedIn']) && $_SESSION['mac-bank']['loggedIn'] != true)) { ?>
  ...
  <div class="form-group squish-form">
    ...
    <div class="col-sm-8">
      <input id="prefered_payment_account" name="preferred_payment_account" class="form-control my-form-control" type="text"  required  value="<?= $formVars['preferred_payment_account']?>"  />
    </div>
  </div>

    <input type="hidden" name="user_type" id="user-type" value="clientManaged"/>
<?php } else if(isset($_SESSION['mac-bank']['loggedIn']) && $_SESSION['mac-bank']['loggedIn'] == true) { ?> 
	    ...
	    if(!empty($myAccounts)) {
		    ...
		    foreach($myAccounts as $account) {
			   ...
			   if($selected_preferred_payment_type != '' && $selected_preferred_payment_type == $account['Name'])
			      $doSelected = $selected;
			   echo "<option value='$account[Name]' $doSelected>$account[Name] : $account[Balance]</option>";
		   }
           echo '</select>';
		}
	  ?>	
